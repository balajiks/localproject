...
<head>
    ...
    @if (get_option('site_appleicon') != '')
        ...
        <link rel="apple-touch-icon" sizes="72x72" href="{{ get_option('site_url') }}{{ getStorageUrl(config('system.media_dir').'/'.get_option('site_appleicon')) }}"/>
        <link rel="apple-touch-icon" sizes="114x114" href="{{ get_option('site_url') }}{{ getStorageUrl(config('system.media_dir').'/'.get_option('site_appleicon')) }}"/>
        <link rel="apple-touch-icon" sizes="144x144" href="{{ get_option('site_url') }}{{ getStorageUrl(config('system.media_dir').'/'.get_option('site_appleicon')) }}"/>
    @endif


    <meta name="csrf-token" content="{{ csrf_token() }}">

    <title>{{ config('app.name', 'Embase Work Flow') }}</title>
    ...
    @if (settingEnabled('show_login_image'))
        <style type="text/css">
            .content:before, #login-blur {
                background-image: url("{{ get_option('site_url') }}{{ getStorageUrl(config('system.media_dir').'/'.get_option('login_bg')) }}");
            }
        </style>
    @endif
