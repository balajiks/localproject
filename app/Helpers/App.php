...
function classByName($name)
{
    switch ($name) {
		case 'indexings':
            return new \Modules\Indexings\Entities\Indexing;
            break;
        case 'deals':
...
function embaseindex_section()
{
    return Cache::remember(
        'embase-embaseindex_section',
        now()->addDays(5),
        function () {
            //return Embaseindex_section::where('status', 'Active')->get()->toArray();
			return \App\Entities\Embaseindex_section::where('status', 'Active')->get()->toArray();
        }
    );
}
...
function renderAjaxButtonSquare($text = 'save', $icon = 'fas fa-paper-plane', $overwriteText = false)
{
    return '<button type="submit" class="btn btn-' . get_option('theme_color') . ' formSaving submit"><i class="' . $icon . '"></i> ' . langapp($text, [], app()->getLocale()) . '</button>';
}
