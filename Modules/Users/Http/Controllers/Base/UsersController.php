...
abstract class UsersController extends Controller
{
    ...
    {
        $allowed      = ['deals', 'files', 'overview', 'projects', 'tickets', 'timesheet','whatsapp'];
    ...
    {
        $model = $this->applyFilter()->with(['profile:user_id,job_title,userrole,mobile,city,use_gravatar,avatar']);
        ...
        return  DataTables::eloquent($model)
			...
			->editColumn(
                'userrole',
                function ($user) {  
					$userrole = DB::table('jobtypes')->where('jobtypeid', $user->profile->userrole)->pluck('jobtypename');                 
					return $userrole[0];
                }
            )
            ->editColumn(
            ...
            ->rawColumns(['name', 'chk', 'job_title', 'userrole', 'mobile', 'user'])
            ->make(true);
    }
