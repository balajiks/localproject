    ...
    <section class="vbox">
        <header class="header bg-white b-b b-light">
            <div class="btn-group pull-right">
                <a href="{{ route('leads.index', ['view' => 'table']) }}" data-rel="tooltip" title="Table" data-placement="bottom" class="btn btn-sm btn-default">
                    @icon('solid/th')
                </a>
                <a href="{{ route('leads.index', ['view' => 'kanban']) }}" data-rel="tooltip" title="Kanban" data-placement="bottom" class="btn btn-sm btn-default">
                    @icon('solid/align-justify')
                </a>
                <a href="{{ route('leads.index', ['view' => 'heatmap']) }}" data-rel="tooltip" title="Heatmap" data-placement="bottom" class="btn btn-sm btn-success">
            ...
            <a href="{{  route('leads.create')  }}" data-toggle="ajaxModal" data-rel="tooltip" title="@langapp('create')" class="btn btn-sm btn-{{ get_option('theme_color') }}">
                @icon('solid/plus') @langapp('create')
            </a>
            
            @admin
            <a href="{{ route('settings.stages.show', 'leads') }}" data-toggle="ajaxModal" class="btn btn-sm btn-{{ get_option('theme_color') }} pull-right" data-rel="tooltip" title="@langapp('stages')" data-placement="bottom">
                @icon('solid/cogs')
            </a>
            @endadmin
            ...
            <a href="{{  route('leads.export')  }}" title="@langapp('export')  " class="btn btn-sm btn-{{ get_option('theme_color') }}">
                                            ...
                                            <div id="lobilist-list-0"
                                                ...
                                                <div class="lobilist-body scrumboard slim-scroll" data-disable-fade-out="true"
                                                    ...
                                                    <ul class="lobilist-items ui-sortable list"
                                                        ...
                                                        <li id="{{ $lead->id }}" draggable="true" class="lobilist-item kanban-entry grab {{ !is_null($lead->unsubscribed_at) ? 'subscribe-bg' : '' }}">
                                                            <div class="lobilist-item-title text-ellipsis m-l-xs font14">
                                                                <a href="{{  route('leads.view', ['id' => $lead->id])  }}" class="">{{ $lead->name }}</a>
                                                                @if($lead->has_email)
                                                ...
                                                <div class="lobilist-footer">
                                                    ...
                                                    <strong class="pull-right">{{ $leadCounter }} Lead(s)</strong>
                                                </div>
                                            </div>
                                        </div>
                                        @endforeach
