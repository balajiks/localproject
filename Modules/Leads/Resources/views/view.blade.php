            ...
            <div class="row m-t-sm">
                <div class="col-sm-12 m-b-xs">
                    <p class="h3">{{ $lead->name }}
                        ...
                        <a class="btn btn-sm btn-{{ get_option('theme_color') }} pull-right" data-toggle="ajaxModal" data-rel="tooltip"  data-placement="bottom" href="{{  route('calendar.reminder', ['module' => 'leads', 'id' => $lead->id])  }}" title="@langapp('set_reminder')  ">
                            @icon('solid/clock')
                        </a>
                        @endcan
                        @can('leads_update')
                        ...
                        @endcan
                        @can('deals_create')
                        <a href="{{ route('leads.convert', ['id' => $lead->id]) }}"
                            class="btn btn-{{ get_option('theme_color') }} btn-sm pull-right" data-rel="tooltip"
            ...
            <div class="sub-tab m-b-10">
                <ul class="nav pro-nav-tabs nav-tabs-dashed">
                    <li class="{{  ($tab == 'overview') ? 'active' : '' }}">
                        <a href="{{  route('leads.view', ['id' => $lead->id, 'tab' => 'overview'])  }}">@icon('solid/home') @langapp('overview')  </a>
                    </li>

                    <li class="{{  ($tab == 'calls') ? 'active' : ''  }}">
                                <a href="{{  route('leads.view', ['id' => $lead->id, 'tab' => 'calls'])  }}">@icon('solid/phone') @langapp('calls')  
                                </a>
                            </li>
                    ...
                    <li class="{{  ($tab == 'whatsapp') ? 'active' : ''  }}">
                        <a href="{{  route('leads.view', ['id' => $lead->id, 'tab' => 'whatsapp'])  }}">
