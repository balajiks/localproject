...
class ImportDataJob implements ShouldQueue
{
    ...
    protected function import()
    {
        ...
        try {
            ...
            if (isset($data['tbl_payment_methods'])) {
                foreach ($data['tbl_payment_methods'] as $method) {
                    AcceptPayment::firstOrCreate(['method_name' => $method['method_name']]);
                }
            }
            if (isset($data['tbl_departments'])) {
                foreach ($data['tbl_departments'] as $dept) {
                    Department::firstOrCreate(['deptname' => $dept['deptname']]);
                }
            }
            if (isset($data['tbl_tax_rates'])) {
                foreach ($data['tbl_tax_rates'] as $rate) {
                    TaxRate::firstOrCreate(['name' => $rate['tax_rate_name']], ['rate' => $rate['tax_rate_percent']]);
                }
            }
            if (isset($data['tbl_users'])) {
                foreach ($data['tbl_users'] as $user) {
                    $member = User::firstOrCreate(['email' => $user['email']], [
                        'username'          => $user['username'],
                        'password'          => $user['password'],
                        'name'              => $data['tbl_account_details']->where('user_id', $user['id'])->first()['fullname'],
                        'email_verified_at' => now()->toDateTimeString(),
                        'calendar_token'    => str_random(60),
                    ]);
                    $p = $data['tbl_account_details']->where('user_id', $user['id'])->first();
                    ...
                    $member->syncRoles($data['tbl_roles']->where('r_id', $user['role_id'])->first()['role']);
                    if (isset($data['tbl_notes'])) {
                        foreach ($data['tbl_notes']->where('owner', $user['id']) as $note) {
                            $member->notes()->create([
                                'user_id'     => $member->id,
                                'name'        => $note['name'],
            ...
            if (isset($data['tbl_companies'])) {
                foreach ($data['tbl_companies'] as $client) {
                    ...
                    if (isset($data['tbl_account_details'])) {
                        foreach ($data['tbl_account_details']->where('company', $client['co_id']) as $contact) {
                            $u = User::where('email', $data['tbl_users']->where('id', $contact['user_id'])->first()['email'])->first();
                            $u->profile()->update(['company' => $company->id]);
                        }
                    }
                    // Import client invoices
                    if (isset($data['tbl_invoices'])) {
                        foreach ($data['tbl_invoices']->where('client', $client['co_id']) as $invoice) {
                            $inv = Invoice::create([
                            ...
                            if (isset($data['tbl_items'])) {
                                foreach ($data['tbl_items']->where('invoice_id', $invoice['inv_id']) as $item) {
                                    $inv->items()->create([
                            ...
                            if (isset($data['tbl_payments'])) {
                                foreach ($data['tbl_payments']->where('invoice', $invoice['inv_id']) as $payment) {
                                    $inv->payments()->create([
                    ...
                    // Import estimates
                    if (isset($data['tbl_estimates'])) {
                        foreach ($data['tbl_estimates']->where('client', $client['co_id']) as $estimate) {
                            $est = Estimate::create([
                            ...
                            if (isset($data['tbl_estimate_items'])) {
                                foreach ($data['tbl_estimate_items']->where('estimate_id', $estimate['est_id']) as $i) {
                                    $est->items()->create([
                    ...
                    if (isset($data['tbl_expenses'])) {
                        foreach ($data['tbl_expenses']->where('project', 0)->where('client', $client['co_id']) as $exp) {
                            Expense::create([
                                ...
                                'category'     => Category::where('name', $data['tbl_categories']->where('id', $exp['category'])->first()['cat_name'])->first()->id,
                    ...
                    // Import projects
                    if (isset($data['tbl_projects'])) {
                        foreach ($data['tbl_projects']->where('client', $client['co_id']) as $project) {
                            $pr = Project::create([
                            ...
                            // Create team
                            if (isset($data['tbl_assign_projects'])) {
                                foreach ($data['tbl_assign_projects']->where('project_assigned', $project['project_id']) as $agent) {
                                    $pr->assignees()->create([
                                        'user_id' => $agent['assigned_user'] > 0 ? User::where('email', $data['tbl_users']->where('id', $agent['assigned_user'])->first()['email'])->first()->id : 1,
                                    ]);
                                }
                            }
                            ...
                            if (isset($data['tbl_expenses'])) {
                                foreach ($data['tbl_expenses']->where('project', $project['project_id']) as $ex) {
                                    $pr->expenses()->create([
                                        'code'         => generateCode('expenses'),
                                        ...
                                        'category'     => Category::where('name', $data['tbl_categories']->where('id', $ex['category'])->first()['cat_name'])->first()->id,
                            ...
                            if (isset($data['tbl_milestones'])) {
                                foreach ($data['tbl_milestones']->where('project', $project['project_id']) as $milestone) {
                            ...
                            if (isset($data['tbl_tasks'])) {
                                foreach ($data['tbl_tasks']->where('project', $project['project_id']) as $task) {
                                    $t = $pr->tasks()->create([
                                        'name'            => $task['task_name'],
                                    ...
                                    // Create team
                                    if (isset($data['tbl_assign_tasks'])) {
                                        foreach ($data['tbl_assign_tasks']->where('task_assigned', $task['t_id']) as $team) {
                                            $t->assignees()->create([
                                                'user_id' => $team['assigned_user'] > 0 ? User::where('email', $data['tbl_users']->where('id', $team['assigned_user'])->first()['email'])->first()->id : 1,
                                            ]);
                                        }
                                    }
                                    // Import task timesheets
                                    if (isset($data['tbl_tasks_timer'])) {
                                        foreach ($data['tbl_tasks_timer']->where('task', $task['t_id']) as $timer) {
                                            $staff = $timer['user'] == 0 || is_null(optional(User::where('email', $data['tbl_users']->where('id', $timer['user'])->first()['email'])->first())->id) ? 1 : User::where('email', $data['tbl_users']->where('id', $timer['user'])->first()['email'])->first()->id;
                                    ...
                                    if (isset($data['tbl_comments'])) {
                                        foreach ($data['tbl_comments']->where('task_id', $task['t_id']) as $c) {
                                            $t->comments()->create([
                                                'user_id'    => $c['posted_by'] > 0 ? User::where('email', $data['tbl_users']->where('id', $c['posted_by'])->first()['email'])->first()->id : 1,
                                                'message'    => $c['message'],
                                    ...
                                    // Import task files
                                    if (isset($data['tbl_task_files'])) {
                                        foreach ($data['tbl_task_files']->where('task', $task['t_id']) as $f) {
                                            $t->files()->create([
                                                ...
                                                'user_id'      => $f['uploaded_by'] > 0 ? User::where('email', $data['tbl_users']->where('id', $f['uploaded_by'])->first()['email'])->first()->id : 1,
                                            ]);
                                        }
                                    }
                                }
                            ...
                            if (isset($data['tbl_todo'])) {
                                foreach ($data['tbl_todo']->where('project', $project['project_id']) as $todo) {
                                    $pr->todos()->create([
                                        ...
                                        'user_id'     => $todo['saved_by'] > 0 ? User::where('email', $data['tbl_users']->where('id', $todo['saved_by'])->first()['email'])->first()->id : 1,
                                        'reminded_at' => now()->toDateTimeString(),
                                    ]);
                                }
                            }
                            ...
                            if (isset($data['tbl_comments'])) {
                                foreach ($data['tbl_comments']->where('project', $project['project_id']) as $comment) {
                                    $pr->comments()->create([
                                        'user_id'    => $comment['posted_by'] > 0 ? User::where('email', $data['tbl_users']->where('id', $comment['posted_by'])->first()['email'])->first()->id : 1,
                            ...
                            if (isset($data['tbl_bugs'])) {
                                foreach ($data['tbl_bugs']->where('project', $project['project_id']) as $issue) {
                                    $i = $pr->issues()->create([
                                        ...
                                        'user_id'         => $issue['reporter'] > 0 ? User::where('email', $data['tbl_users']->where('id', $issue['reporter'])->first()['email'])->first()->id : 1,
                                        'status'          => optional(Status::where('status', $issue['bug_status'])->first())['id'],
                                    ]);

                                    if (isset($data['tbl_bug_comments'])) {
                                        foreach ($data['tbl_bug_comments']->where('bug_id', $issue['bug_id']) as $bug) {
                                            $i->comments()->create([
                                                'user_id' => $bug['comment_by'] > 0 ? User::where('email', $data['tbl_users']->where('id', $bug['comment_by'])->first()['email'])->first()->id : 1,
                                                'message' => $bug['comment'],
                                            ]);
                                        }
                                    }
                            ...
                            if (isset($data['tbl_files'])) {
                                foreach ($data['tbl_files']->where('project', $project['project_id']) as $file) {
                                    $pr->files()->create([
                                        ...
                                        'user_id'      => $file['uploaded_by'] > 0 ? User::where('email', $data['tbl_users']->where('id', $file['uploaded_by'])->first()['email'])->first()->id : 1,
                                    ]);
                                }
                            }
                            // Import project timesheets
                            if (isset($data['tbl_project_timer'])) {
                                foreach ($data['tbl_project_timer']->where('project', $project['project_id']) as $entry) {
                                    $staff = $entry['user'] == 0 || is_null(optional(User::where('email', $data['tbl_users']->where('id', $entry['user'])->first()['email'])->first())->id) ? 1 : User::where('email', $data['tbl_users']->where('id', $entry['user'])->first()['email'])->first()->id;
                                    $pr->timesheets()->create([
            ...
            // Import Tickets
            if (isset($data['tbl_tickets'])) {
                foreach ($data['tbl_tickets'] as $ticket) {
                    $support = Ticket::create([
                        'code'       => generateCode('tickets'),
                        'subject'    => $ticket['subject'],
                        'user_id'    => $ticket['reporter'] > 0 ? User::where('email', $data['tbl_users']->where('id', $ticket['reporter'])->first()['email'])->first()->id : 0,
                        'body'       => $ticket['body'],
                        'status'     => Status::where('status', $ticket['status'])->first()->id,
                        'department' => Department::where('deptname', $data['tbl_departments']->where('deptid', $ticket['department'])->first()['deptname'])->first()->deptid,
                        'created_at' => validateDate($ticket['created']) ? dateParser($ticket['created'])->toDateTimeString() : now(),
                    ]);
                    if (isset($data['tbl_ticketreplies'])) {
                        foreach ($data['tbl_ticketreplies']->where('replierid', $user['id']) as $reply) {
