<div class="col-lg-12">
	<section class="panel panel-default">
		<form id="frm-indexing" action="{{ route('indexings.bulk.email') }}" method="POST">
			<input type="hidden" name="_token" value="{{ csrf_token() }}">
			<input type="hidden" name="module" value="indexings">
			<div class="table-responsive">
				<table class="table table-striped" id="indexings-table">
					<thead>
						<tr>
							<th class="hide"></th>
							<th class="no-sort">
...
<script>
	$(function() {
	$('#indexings-table').DataTable({
	processing: true,
	serverSide: true,
	ajax: {
		url: '{{ route('indexings.data') }}',
		type: 'POST',
        headers: {
...
if($(this).attr("value")=="bulk-delete"){
    var form = $("#frm-indexing").serialize();
    axios.post('{{ route('indexings.bulk.delete') }}', form)
    .then(function (response) {
    toastr.warning(response.data.message, '@langapp('response_status') ');
    window.location.href = response.data.redirect;
    })
