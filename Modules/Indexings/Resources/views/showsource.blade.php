...
<aside class="b-l bg" id="">
  <section class="vbox"><br />
    <div class="panel-group m-b" id="accordion2">
      <ul class="list no-style" id="responses-list">
        <li class="panel btn-primary" id="tokenize">
          <div class="panel-heading">@icon('solid/cogs') @langapp('pdfviewer') <span class="btn btn-warning btn-xs">JobID: {{ $jobsourceinfo[0]->id }}</span> &nbsp;&nbsp; <span class="btn btn-warning btn-xs">PUI: {{ $jobsourceinfo[0]->pui }}</span> </div>
        </li>
      </ul>
    </div>
...
function passsentences(selectedval,term,termType,score){
   if(selectedval !='') {
		$("#preloader").show();	
		axios.post('{{ get_option('site_url') }}api/v1/indexings/ajax/esvsentences', {
