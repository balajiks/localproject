...
<div class="wrapper b-b b-light">
    ...
    <a href="{{ route('indexings.emails', ['id' => $email->id, 'action' => 'delete'])  }}" data-toggle="ajaxModal" class="pull-right text">@svg('solid/trash-alt')
    </a>
        ...
        <section class="media-body display-block">
            ...
            <div class="padder">
                <div class="panel bg-light">
                    <div class="panel-body">
                        <h4 class="font-thin ">Type email reply (Will be sent to {{ $email->indexing->email }})</h4>

                        {!! Form::open(['route' => 'indexings.emailReply', 'class' => 'm-b-none ajaxifyForm', 'novalidate']) !!}

                        <input type="hidden" name="indexing_id" value="{{  $indexing->id  }}">
                        <input type="hidden" name="reply_id" value="{{  $email->id  }}">
                        <input type="hidden" name="to" value="{{  $email->indexing->email  }}">
