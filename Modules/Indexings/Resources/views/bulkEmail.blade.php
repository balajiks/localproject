      ...
      <section class="vbox">
        <header class="header bg-white b-b clearfix">
          <div class="row m-t-sm">
            <div class="col-sm-12 m-b-xs">
              <p class="h3">@langapp('send_email_to_indexings')</p>
            </div>
          </div>
        </header>
        <section class="scrollable wrapper bg">
          <div class="panel panel-body">
            ...
            <header class="panel-heading"><span class="text-dracula">{{ $indexings->count() }} @langapp('indexings')</span></header>
            {!! Form::open(['route' => 'indexings.bulk.send', 'class' => 'bs-example ajaxifyForm', 'data-toggle' => 'validator']) !!}
            
            <div class="panel-body">
              
              <div class="form-group">
                <label class="control-label">@langapp('indexings') @required</label>
                <select class="select2-option width100" multiple="multiple" name="indexings[]">
                  @foreach ($indexings  as $indexing)
                  <option value="{{ $indexing->id }}" selected>{{ $indexing->name }} &laquo;{{ $indexing->email }}&raquo;</option>
                  @endforeach
                </select>
              </div>
              <div class="form-group">
