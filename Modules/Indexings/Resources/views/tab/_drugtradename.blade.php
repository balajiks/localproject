  ...
  <div class="col-lg-12">
    <section class="panel panel-default">
      <header class="panel-heading">
        <header class="btn btn-primary btn-sm" style="margin-left:20px;"> @icon('solid/exclamation-circle') @langapp('drugtradeinfo')  &nbsp;&nbsp;[ <span class="btn btn-warning btn-xs">@langapp('drugtradename') </span> ]</header>
      </header>
      @php 
      ...
      <div class="panel-body">
        <div class="col-md-12"> 
              {!! Form::open(['route' => 'indexings.api.savedrugtradename', 'id' => 'createDrugTradeName', 'class' => 'bs-example form-horizontal m-b-none']) !!}
              <div class="col-md-6">
                ...
                <div class="form-group">
                  <div class="col-lg-2">
                    ...
                    <span class="label-text text-info">@langapp('ma')</span></label>
                  </div>
                  <div class="col-lg-2">
                    <label>
                    <input type="radio" id="txtdrugtr" name="termDTNindexing" value="0">
                    <span class="label-text text-info">@langapp('tr')</span></label>
                  </div>
                </div>
                <div class="form-group">
                  <div class="col-lg-12">
                    <div class="form-group" id="divmaname">
                      <div class="col-lg-6">
                        ...
                        <input type="text" class="form-control" id="txtdrugmanufacturename" placeholder="Drug Manufacture Name" name="txtdrugmanufacturename" disabled="disabled" value="{{ @$data['txtdrugmanufacturename']}}" autocomplete="off" />
                        <div id="suggesstiondrungtrade-box"></div>
                      </div>
                      <div class="col-lg-5">
                        <label class="control-label" for="fname">Country Code</label>
                        <input type="text" class="form-control" id="txtcountrycode" placeholder="Country code" name="txtcountrycode" disabled="disabled" value="{{ @$data['txtcountrycode']}}" autocomplete="off" />
                    ...
                    <div class="form-group" id="divtrname">
                      <div class="col-lg-6 input_fields_wrap" >
                        ...
                        <input type="text" class="form-control" id="txtdrugtradename" placeholder="Drug Trade Name" name="txtdrugtradename[]" disabled="disabled" value="{{ @$data['txtdrugtradename']}}" autocomplete="off" />
                      </div>
                      <div class="col-lg-2">
...
{!!  renderAjaxButtonSquare('save')  !!} </div>
			  ...
			  <div class="col-md-6">
			  <div class="form-group">
					<div class="drugtrade-list" id="nestable"> @widget('Indexings\ShowDrugTradeName', ['user_id' => \Auth::id(), 'jobid' => $jobdata->id, 'orderid' => $jobdata->orderid]) </div>
...
function ajaxtradename(val){
   if(val !='') {
	...
	$("#ajaxterm-"+val).addClass("activeli");
		...
		axios.post('{{ get_option('site_url') }}api/v1/indexings/ajax/tradenamedata', {
			selectedterm: val
		})
		.then(function (response) {
...
function selectedCountry(name){
	...
	$(add_button).click(function(e){ 
		...
		if(x < max_fields){
			x++; 
				$(wrapper).append('<div><input type="text" class="form-control" id="txtdrugtradename" placeholder="Drug Trade Name" name="txtdrugtradename[]" value="{{ @$data['txtdrugtradename']}}" autocomplete="off" /><a href="#" class="remove_field btn btn-danger btn-xs">Remove</a></div>'); 
		}
	});
