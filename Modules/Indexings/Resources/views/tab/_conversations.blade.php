  ...
  <div class="chat-history">
    <article class="chat-item chat-message clearfix" id="chat-form">
      ...
      <section class="chat-body">
        {!! Form::open(['route' => ['indexings.email', $indexing->id], 'class' => 'bs-example ajaxifyForm m-b-none', 'id' => 'sendMail', 'data-toggle' => 'validator', 'files' => true]) !!}
        <input type="hidden" name="to" value="{{ $indexing->id }}">
        <input type="hidden" name="from" value="{{ Auth::id() }}">
        <div class="form-group">
          <label class="control-label">@langapp('subject') @required</label>
          <input type="text" name="subject" value="{{ optional($indexing->emails->last())->subject }}" class="form-control">
        </div>
    ...
    <section class="chat-list panel-body" id="msg-list">
      ...
      <ul id="indexingConversations" class="list">
        @foreach($indexing->emails as $message)
        
        <article id="message-{{$message->id}}" class="chat-item {{ $message->from == $indexing->sales_rep ? 'left' : 'right' }}">
          <a href="#" class="pull-{{ $message->from == $indexing->sales_rep ? 'left' : 'right' }} thumb-sm avatar">
          <img src="{{ $message->from == $indexing->sales_rep ? $message->sender->photo : $indexing->photo }}" class="img-circle"></a>
          ...
          <section class="chat-body comment-body">
            <div class="panel b-light m-b-none" >
              ...
              <div class="panel-body">
                <span class="arrow {{ $message->from === $indexing->sales_rep ? 'left' : 'right' }}"></span>
                ...
                <span class="message-data-name text-muted" >
                  ...
                  @if($message->from == $indexing->sales_rep)
            ...
            <small class="text-muted message-data-time small">@icon('solid/user-circle') {{ $indexing->name }}
...
url = '{{ route('indexings.email.delete') }}';
