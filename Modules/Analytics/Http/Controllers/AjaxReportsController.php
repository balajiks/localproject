...
class AjaxReportsController extends Controller
{
	...
	/**
     * Display a listing of the indexings.
     *
     * @return \Illuminate\Http\JsonResponse
     */
    public function indexings(Indexing $indexing)
    {
        $range      = explode('-', $this->request->range);
        $start_date = date('Y-m-d', strtotime($range[0]));
        $end_date   = date('Y-m-d', strtotime($range[1]));
        $this->request->request->add(['date_range' => [$start_date, $end_date]]);
        $indexings = $lead->apply($this->request->except(['range']))->with(['AsSource', 'status'])->get();
        $html  = view('analytics::_ajax._indexings', compact('indexings'))->render();
    ...
    public function feedback(Feedback $feedback)
    {
        ...
        $end_date   = date('Y-m-d', strtotime($range[1]));
        $this->request->request->add(['date_range' => [$start_date, $end_date]]);
        $ratings = $feedback->apply($this->request->except('range'))->where('reviewable_type', Project::class)->with('user:id,username,name')->get();
        $html    = view('analytics::_ajax._feedbacks', compact('ratings'))->render();
